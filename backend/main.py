"""
DermScan API v4 — FastAPI backend
"""
...
import os
print("MODEL_PATH =", os.getenv("MODEL_PATH"))   # ← add this
print("FILES IN ./models/ =", os.listdir("./models") if os.path.exists("./models") else "FOLDER NOT FOUND")
...
def _simulate_multiclass(prob):
    ...
    return sorted([{"code":k,"name":MULTI_CLASS_FULL[k],"probability":round(v,4),
                    "malignant":k in MALIGNANT_CLASSES} for k,v in norm.items()],
                  key=lambda x:x["probability"],reverse=True)
@app.get("/")
def root():
    return {
        "message": "DermScan API v4.0.0 is running",
        "docs": "/docs",
        "redoc": "/redoc",
        "health": "/api/health"
    }
