...
def get_analytics(db: Session, owner_id: str = None) -> dict:
    """
    Returns aggregate statistics. If owner_id is set, scoped to that user's patients.
    """
    # Base query
    visits_q = db.query(Visit)
    if owner_id:
        patient_ids = [p.id for p in db.query(Patient).filter(
            Patient.owner_id == owner_id).all()]
    ...
    total      = len(visits)
    malignant  = sum(1 for v in visits if v.prediction and "Consultation" in v.prediction)
    benign     = total - malignant
    confidences = [v.confidence for v in visits if v.confidence is not None]
    avg_conf   = round(sum(confidences) / len(confidences), 4) if confidences else 0.0

    # Risk distribution from change_json
